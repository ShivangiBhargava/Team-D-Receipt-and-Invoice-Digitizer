...
# In a real production app, these would be stored in a database table (e.g., vendor_templates)
# containing regex rules, expected tax rates, and standardized naming conventions.
VENDOR_TEMPLATES = {
    "coffee house": {
    ...
    "walmart": {
        ...
        "tax_rate": None, # Varies too much, don't assume tax rate
        "aliases": ["walmart supercenter", "wm supercenter"]
    },
    # Add the vendor from the example image
    "coffee house inc.": {
         "standardized_name": "Coffee House",
         "tax_rate": 0.0825,
         "aliases": []
...
def calculate_parsing_score(data):
    ...
    """
    score = 0
    # Critical fields carry more weight
...
def find_matching_template(merchant_name):
    ...
    for vendor_key, template_data in VENDOR_TEMPLATES.items():
        # Check if base key is in the extracted name (e.g. "coffee house" in "the coffee house inc")
...
def apply_template_parsing(standard_data):
    ...
    Returns the refined data and a boolean indicating if a template was applied.
    """
    ...
    if template:
        ...
        if total > 0 and (tax == 0 or tax is None) and known_rate is not None:
            ...
            refined_data['tax_rate_applied'] = known_rate # Marker for UI
...
def main():
    ...
    # New session state for parsing comparison
                    ...
                    with st.spinner("Running OCR, Standard AI Parsing, and Template Matching..."):
                        ...
                        if structured_data_standard:
                            ...
                            receipt_data_standard = {
                                ...
                                "date": structured_data_standard.get('date'), # Keep None if missing for simulation
                            ...
                            # 2. Template-Based Parsing (Refinement)
                            ...
                            # Store comparison data for Tab 2
                            ...
                            # Use Template data as the "final" data for validation and saving
                            final_receipt_data = receipt_data_template
                            # Ensure date is not None for DB save
    ...
    with tab_validation:
        # --- TEMPLATE PARSING COMPARISON UI ---
        st.markdown("""
        ...
        with st.container():
            ...
            if comparison_data:
                ...
                # Format Tax for Template Box (Highlight if applied)
            ...
            st.markdown('</div>', unsafe_allow_html=True) # End comparison-container
        ...
        if st.session_state['current_receipt']:
            # Use the finalized data (which is template-parsed if available)
